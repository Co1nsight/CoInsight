...
public class HuggingFaceClient {
    ...
    // BERT 모델은 512 토큰 제한 (한국어는 토큰당 문자가 적으므로 1200자로 설정)
    ...
    public SentimentAnalysisResult analyzeSentiment(String text) {
        ...
        log.debug("Split text into {} chunks for Korean sentiment analysis", chunks.size());
    ...
    private SentimentAnalysisResult analyzeChunk(String chunk) {
        ...
        try {
            ...
            log.debug("HuggingFace API raw response: {}", response.getBody());
    ...
    private List<String> splitIntoChunks(String text) {
        ...
        while (start < text.length()) {
            ...
            if (end < text.length()) {
                ...
                int lastKoreanPeriod = text.lastIndexOf("다. ", end);
                int lastNewline = text.lastIndexOf("\n", end);
                int breakPoint = Math.max(Math.max(lastPeriod, lastKoreanPeriod), lastNewline);
